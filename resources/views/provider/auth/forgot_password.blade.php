...
<div class="page-content">
	...
	<div class="prov-login">   
        ...
        <form class="login-form" action="{{$disable ? '' : route('provider.send_mail')}}" method="post">
        	@csrf
		    
		    <div class="form-group">
		     	<label for="email">{{tr('email_address')}} *</label>
		      	<input type="email" name="email" class="form-control" id="email" placeholder="{{tr('email_address')}}" value="{{old('email')}}" required>
		    </div>
		    ...
		    @endif
		    <hr>
		    <h4 class="m-0 text-center captalize">{{tr('already_have_an_account')}}? <a href="{{route('provider.login')}}" class="bold-cls close-login" style="color: orange"> {{tr('login')}}</a></h4>
